...
def show_status():
    try:
        ...
        # Ошибка уже распечатается в db_client.py
        return

    # 1. Системная информация о БД
    ...
    # 2. Список таблиц и статистика строк
    try:
        cur.execute("""
            SELECT table_name 
    ...
    # 3. Бизнес-метрики Интернет-магазина
    console.print("\n[bold yellow]🛍️ Аналитика интернет-магазина (ShopDB):[/bold yellow]")
    try:
        # Всего заказов и выручка (исключая отмененные)
        cur.execute("SELECT COUNT(*), COALESCE(SUM(total_amount), 0) FROM orders WHERE status != 'Отменен';")
        ...
        # Товары на исходе (остаток < 15)
        cur.execute("SELECT name, stock, price FROM products WHERE stock < 15 ORDER BY stock ASC LIMIT 3;")
        low_stock = cur.fetchall()
        
        metric_table = Table(show_header=False, box=None)
        metric_table.add_row("[bold]Всего клиентов:[/bold]", f"{customer_count} чел.")
        metric_table.add_row("[bold]Активных заказов (кроме отмененных):[/bold]", f"{order_count} шт.")
        metric_table.add_row("[bold]Общая выручка:[/bold]", f"[bold green]{total_revenue:,.2f} руб.[/bold green]")
        
        console.print(metric_table)
        
        if low_stock:
            low_stock_table = Table(title="\n⚠️ Внимание! Товары на исходе (критический остаток)")
            low_stock_table.add_column("Товар", style="yellow")
            low_stock_table.add_column("Остаток на складе", justify="right", style="red")
        ...
        console.print(f"[red]Не удалось получить бизнес-метрики. Проверьте структуру таблиц: {e}[/red]")
